...
function Feedback() {
  ...
  return (
    <div className='p-10'>
      <h2 className='text-3xl mb-2 font-bold text-green-500'>
        Congratulation!
      </h2>
      ...
      <div>
        {feedback.length > 0 ?
          ...
          <>
            <h2 className='text-lg my-3 text-primary'>Your Overall Interview Rating: <strong>{averageRating.toFixed(1)}</strong></h2>
            ...
            {feedback.map((item, idx) => (
              <Collapsible key={idx} className='mt-7'>
                <CollapsibleTrigger className='p-2 flex justify-between bg-secondary rounded-lg my-3 text-left w-full'>{item.question} <ChevronsUpDown /></CollapsibleTrigger>
                <CollapsibleContent>
